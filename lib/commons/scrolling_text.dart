...
class ScrollingTextState extends State<ScrollingText>
      ...
      timer = Timer.periodic(Duration(milliseconds: _timerRest), (timer) {
        ...
        if (pixels + _moveDistance >= maxScrollExtent) {
          if (widget.scrollAxis == Axis.horizontal) {
            position =
                (maxScrollExtent - screenWidth * widget.ratioOfBlankToScreen + widgetWidth)
                    / 2 - widgetWidth + pixels - maxScrollExtent;
          } else {
            position =
                (maxScrollExtent - screenHeight * widget.ratioOfBlankToScreen + widgetHeight)
                    / 2 - widgetHeight + pixels - maxScrollExtent;
          }
  ...
  Widget getBothEndsChild() {
    if (widget.scrollAxis == Axis.vertical) {
      ...
      return Center(
        child: Text(newString, style: widget.textStyle, textAlign: TextAlign.center),
      );
    }
    return Center(
